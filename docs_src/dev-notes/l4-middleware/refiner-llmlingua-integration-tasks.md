...
2. 更新 `sageRefiner/sage_refiner/algorithms/__init__.py`:
   - 移除 adaptive 相关导入和导出
   - 移除 llmlingua 相关导入和导出 (旧版)

3. 更新 `sage_refiner/__init__.py` (如果存在相关导出)
...
3. 更新 `implementations/pipelines/__init__.py` (如有相关导出)

4. 更新 `implementations/__init__.py` (如有相关导出)
...
4. 更新 `algorithms/__init__.py` 添加导出
...
2. 更新 `sage_refiner/__init__.py` 导出新的 Compressor 和 Operator
...
**操作清单**:
1. 创建测试文件:
   `packages/sage-benchmark/tests/benchmark_refiner/test_longllmlingua.py`
...
2. **无本地源码依赖**: 不再依赖 LLMLingua-main 文件夹，已删除
3. **论文 Baseline 配置**: `DEFAULT_LONG_LLMLINGUA_CONFIG` 配置符合 Jiang et al. (2024) 论文
4. **设备兼容**: 确保 `device_map` 参数支持 "cuda", "cpu", "cuda:0" 等格式
5. **测试模式**: Pipeline 需支持 `SAGE_TEST_MODE=true` 环境变量
