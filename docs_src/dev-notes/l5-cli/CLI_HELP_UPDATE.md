...
**Summary**: SAGE CLI 帮助信息更新总结 - 对齐 `sage` (packages/sage-cli) 与 `sage-dev` (packages/sage-tools)
的帮助文本
...
1. `sage-dev` → 看到欢迎信息和快速开始
1. `sage-dev --help` → 看到所有命令组
1. `sage-dev quality --help` → 看到质量检查的所有命令
...
- [COMMAND_REORGANIZATION_SUMMARY.md](./COMMAND_REORGANIZATION_SUMMARY.md#-%E6%97%A7--%E6%96%B0-%E5%91%BD%E4%BB%A4%E6%98%A0%E5%B0%84)
  \- 迁移映射表
