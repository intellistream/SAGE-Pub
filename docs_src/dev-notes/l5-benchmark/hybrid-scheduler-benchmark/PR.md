...
本 PR 为 sageLLM Control Plane 的调度策略实现完整的 Benchmark 评测框架，支持 LLM 调度和混合调度（LLM + Embedding）两种模式，并提供自动可视化和预定义实验功能。
...
1. **性能优化**: 大规模负载下的内存优化
2. **更多图表**: 添加 Plotly 交互式图表支持
3. **CI 集成**: 添加 benchmark 回归测试到 CI
4. **实验扩展**: 添加更多预定义实验（fairness, cost, tail latency）
