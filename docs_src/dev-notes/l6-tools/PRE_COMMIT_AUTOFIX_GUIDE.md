...
**原因**：格式化或 import 排序让原本被隐藏的类型不一致暴露出来（例如 `Optional[str]` 被 `black` 展开成 `str | None` 之后，赋值语句突然违反类型约束）。
...
2. **新错误并非新引入**：它们原本就存在，只是以前未被静态检查捕获。
3. **错误数量会递减**：
...
| 只对修改文件跑 mypy | `git status --short | awk '{print $2}' | xargs python -m mypy --ignore-missing-imports` |
...
2. 使用 `sage-dev quality fix` / `sage-dev quality check` 替换零散的脚本。
3. `pre-commit install` 后首次运行时间可能较长（会安装工具）。
4. 真正需要跳过 hook 时，一定在提交信息说明原因，后续补齐。
