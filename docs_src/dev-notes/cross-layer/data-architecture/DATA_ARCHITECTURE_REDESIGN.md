...
当前 `sageData` (主要位于 `packages/sage-benchmark/src/sage/data`) 目录结构缺乏统一规范，文件夹组织混乱。为了支持 SAGE 作为一个通用 AI
数据处理框架的延展性，我们需要重构数据管理架构。

**核心目标：**

1. **清理与规范化**：解决文件夹混乱问题。
1. **数据共享**：最大化数据集的复用，避免重复存储。
1. **用途隔离**：不同研究方向（如 LibAMM, NeuroMem, RAG）的用户只关注与自己相关的数据视图。
1. **高延展性**：支持快速添加新数据集和定义新用途，适应不同实验需求。
...
**方式 A: 动态注册 (推荐)** 用户导入用途模块时，自动注册相关数据集。
...
1. 创建配置文件，列出需要用到的 `sources`。
1. (可选) 添加该用途特有的预处理脚本。

## 6. 迁移计划 (Migration Plan)

1. **Phase 1**: 建立 `sources` 和 `usages` 目录结构。
1. **Phase 2**: 将现有 `sage/data/` 下的散乱文件夹 (e.g., `bbh`, `locomo`) 移动到 `sources/` 下。
1. **Phase 3**: 为现有主要用途 (LibAMM, RAG) 创建 `usages` 映射。
1. **Phase 4**: 更新 `__init__.py` 提供向后兼容的导入路径 (Deprecation Warning)。
