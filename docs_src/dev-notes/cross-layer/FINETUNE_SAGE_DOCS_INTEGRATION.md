...
2. `POST /api/finetune/use-as-backend`
   - 验证任务状态
   - 注册到 vLLM Registry
   - 切换模型并更新环境变量
...
**Q: 文档下载失败怎么办？**
A: 检查网络连接，确保能访问 GitHub。可配置代理：
```bash
git config --global http.proxy http://127.0.0.1:7890
```

**Q: 准备数据后没有自动填充文件路径？**
A: API 返回的 `data_file` 已自动设置到 `uploadedFile` 状态，直接提交表单即可。

**Q: 切换后端后 Chat 还是用旧模型？**
A: 检查环境变量 `SAGE_STUDIO_LLM_MODEL`，确保 vLLM Registry 切换成功。

**Q: 训练数据太少效果不好？**
A: SAGE 文档可能只有 100-200 条数据，建议：
