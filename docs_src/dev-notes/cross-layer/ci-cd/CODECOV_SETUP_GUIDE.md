...
This document explains how to set up codecov.io integration for the SAGE project to track test
coverage across all commits and pull requests.
...
1. Navigate to repository list: https://codecov.io/gh/intellistream
1. Find `SAGE` repository and enable it
1. If not listed, click "Add new repository" and select `intellistream/SAGE`

### 2. Get Codecov Token

1. Go to repository settings: https://codecov.io/gh/intellistream/SAGE/settings
1. Navigate to "General" tab
1. Copy the "Repository Upload Token" (format: `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`)
...
1. Value: Paste the token copied from step 2
1. Click "Add secret"
...
1. Look for "Upload Coverage to Codecov" step
1. Verify it completes successfully
1. Visit Codecov dashboard: https://codecov.io/gh/intellistream/SAGE
1. Confirm coverage report appears
...
1. Check token hasn't expired (no expiration for upload tokens)
1. Ensure `coverage.xml` file is generated before upload
1. Check codecov.io service status: https://status.codecov.io
...
1. Check file paths in coverage.xml match repository structure
1. Ensure `codecov.yml` ignore patterns aren't too broad
1. Look at CI logs for pytest execution errors
...
1. Add `?random=<timestamp>` to badge URL temporarily
1. Wait 5-10 minutes for CDN propagation
1. Check codecov dashboard has latest data
...
1. Ensure GitHub App has PR write permissions
1. Check codecov.yml `comment.behavior` setting
1. Verify base branch has coverage data for comparison
