...
1. **kernel.api 和 BaseService 分层不一致**
...
1. ✅ **函数接口最大复用**: 在 L1 层，所有包都可以安全使用
1. ✅ **架构更清晰**: 接口(L1) → 平台服务(L2) → 执行引擎(L3) → 领域组件(L4)
1. ✅ **简化依赖链**: libs 只需 common，不需 platform 或 kernel
1. ✅ **向后兼容**: kernel 保留兼容层，不破坏现有代码
1. ✅ **删除重复代码**: KafkaSource 只保留一个实现

### 缺点/风险

1. ⚠️ **common 包变大**: 从基础工具扩展到函数接口
   - 缓解: functions 作为 core 子模块，职责清晰
1. ⚠️ **短期工作量**: 需要更新 ~32-47 个文件
   - 缓解: 大部分是简单的导入路径替换
1. ⚠️ **测试覆盖**: 需要全面测试确保没有遗漏
   - 缓解: 保留兼容层，渐进式验证
1. ⚠️ **KafkaSource 迁移**: 需要将完整实现从 kernel 迁移到 libs
   - 缓解: 代码已经存在，只需复制粘贴
...
1. **回答问题** - 确定 kafka_source 和 \_internal_print_sink 的处理方式
1. **创建分支** - `feature/kernel-refactoring-1041`

### 执行步骤

1. ✅ Phase 1: 创建 sage-platform/api (1-2 小时)
1. ✅ Phase 2: 更新 sage-kernel 兼容层 (0.5 小时)
1. ✅ Phase 3: 更新 sage-libs 导入 (1 小时)
1. ✅ Phase 4: 更新 sage-middleware 导入 (1 小时)
1. ✅ Phase 5: 更新 pyproject.toml (0.5 小时)
1. ✅ Phase 6: 测试和验证 (2-3 小时)
1. ✅ Phase 7: 更新文档 (1-2 小时)
