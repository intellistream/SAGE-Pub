...
1. **质量提升**: 语义理解能力大幅提升，检索更准确
1. **成本控制**: 可选择免费本地方法 vs 付费 API 方法
1. **可观测性**: 通过 analyze 命令量化对比不同方法

### 对系统的价值

1. **架构统一**: 复用统一的 embedding 系统，减少维护成本
1. **扩展性强**: 新增 embedding 方法自动可用
1. **向后兼容**: 默认使用 hash，不影响现有用户
1. **质量保证**: 自动后备机制确保系统稳定性
...
1. **CLI 参数增强** - `build` 命令新增 2 个 embedding 配置参数
1. **新增分析命令** - `analyze-embedding` 帮助选择最佳方法
1. **环境变量支持** - 通过环境变量配置默认行为
1. **自动后备机制** - 方法失败时自动回退到 hash
1. **完整文档** - 使用指南、最佳实践、性能对比
