...
- **位置**：`tools/install/examination_tools/dependency_verification.sh`
- **函数**：`verify_package_checksum()`
- **使用**：
  ```bash
...
- **选项**：`--verify-deps` 和 `--verify-deps-strict`
- **功能**：综合验证
  - pip 依赖检查 (`pip check`)
  - 安全漏洞扫描 (pip-audit + safety)
  - 包版本兼容性检查
  - 详细报告生成
- **位置**：`tools/install/download_tools/argument_parser.sh`
- **实现**：`perform_deep_verification()`
- **使用**：
  ```bash
...
- **函数**：`verify_pip_packages()` 和 `verify_all_installed_packages()`
- **功能**：验证已安装包的完整性
- **检查项**：
  - 依赖冲突
  - 版本不匹配
...
- **实现**：智能 sudo 使用
  ...
  - 避免整个脚本以 root 运行
  - 提供权限需求透明化
- **位置**：`tools/install/examination_tools/system_deps.sh`
- **代码**：自动检测是否需要 sudo
...
- **GitHub Discussions**：[SAGE 社区](https://github.com/intellistream/SAGE/discussions)
- **Issue Tracker**：[GitHub Issues](https://github.com/intellistream/SAGE/issues)
