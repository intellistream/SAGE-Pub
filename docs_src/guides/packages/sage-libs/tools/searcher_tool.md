...
SearcherTool 是 SAGE 框架中用于执行各类搜索任务的工具集合。它位于 L4 层（sage-middleware），基于 L3 层的 `BaseTool` 基类构建，提供了从学术论文到网页内容的多种搜索能力。
...
2. **结果数量限制**：合理设置 `max_results` 以平衡速度和覆盖率
3. **错误处理**：搜索工具应优雅处理网络错误和 API 限流
4. **结果缓存**：对重复查询考虑缓存机制以提高效率
5. **日志记录**：使用工具内置的 logger 记录搜索行为
